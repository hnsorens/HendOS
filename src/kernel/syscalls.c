...
void sys_mmap()
{
    ...
    // interrupts)
    uint64_t addr = SYS_ARG_1;
    uint64_t length = SYS_ARG_2;
    uint64_t prot = SYS_ARG_3;
    uint64_t flags = SYS_ARG_4;
    uint64_t fd = SYS_ARG_5;
    uint64_t offset = SYS_ARG_6;
    /**
...
void sys_write()
{
    uint64_t out = SYS_ARG_1;
    uint64_t msg = SYS_ARG_2;
    uint64_t len = SYS_ARG_3;
...
{
    // SYSCALL_ARGS(name);
    uint64_t name = SYS_ARG_1;
    uint64_t argc = SYS_ARG_2;
    uint64_t argv = SYS_ARG_3;
...
void sys_dup2()
{
    uint64_t old_fd = SYS_ARG_1;
    uint64_t new_fd = SYS_ARG_2;
...
void sys_open()
{
    uint64_t path = SYS_ARG_1;
    uint64_t perms = SYS_ARG_2;
...
void sys_setpgid()
{
    ...
    uint64_t pid = SYS_ARG_1;
    uint64_t pgid = SYS_ARG_2;
...
{
    uint64_t pid = SYS_ARG_1;
    uint64_t options = SYS_ARG_2;
    uint64_t* status = SYS_ARG_3;

    process_t* process = pid_hash_lookup(PID_MAP, pid);

    if (process->flags & PROCESS_ZOMBIE)
    {
        *((uint64_t*)SYS_ARG_2) = process->status;
        (*CURRENT_PROCESS)->process_stack_signature.rax = process->pid;
        return;
    }
...
void sys_kill()
{
    int64_t pid = SYS_ARG_1;
    uint64_t signal = SYS_ARG_2;
...
{
    uint64_t fd = SYS_ARG_1;
    uint64_t offset = SYS_ARG_2;
    uint64_t whence = SYS_ARG_3;
