...
static int add_entry(ext2_fs_t* fs, uint32_t dir_inode, const char* name, 
                    uint32_t inode_num, uint8_t file_type);
static int remove_entry(ext2_fs_t* fs, uint32_t dir_inode, const char* name);
static int find_entry(ext2_fs_t* fs, uint32_t dir_inode, const char* name, 
                     uint32_t* inode_out, uint8_t* file_type);
static int read_block_pointers(ext2_fs_t* fs, struct ext2_inode* inode, 
                             uint32_t block_idx, uint32_t* blocks, uint32_t count);
static int write_block_pointers(ext2_fs_t* fs, struct ext2_inode* inode, 
                              uint32_t block_idx, uint32_t* blocks, uint32_t count);
static int update_file_size(ext2_fs_t* fs, uint32_t inode_num, struct ext2_inode* inode, 
                           uint32_t new_size);
...
int ext2_init(ext2_fs_t* fs, void* (*read_fn)(uint32_t, uint32_t), 
...
int ext2_file_open(ext2_fs_t* fs, ext2_file_t* file, const char* path) {
    ...
    if (!(file->inode.mode & EXT2_S_IFREG)) { // Not a regular file
        return -1;
    }
...
int ext2_dir_count_entries(ext2_fs_t* fs, uint32_t dir_inode) {
    ...
    while (offset < inode.size) {
        ...
        while (pos < fs->block_size) {
            ...
            if (entry->inode != 0) { // Count only valid entries
                count++;
            }
...
int ext2_dir_iter_start(ext2_fs_t* fs, ext2_dirent_iter_t* iter, const char* path) {
    ...
    if (!(inode.mode & EXT2_S_IFDIR)) { // Not a directory
        return -1;
    }
...
static void* read_block(ext2_fs_t* fs, uint32_t block_num) {
    ...
    return fs->read_sectors(
        fs->start_sector + block_num * (fs->block_size / SECTOR_SIZE),
...
static uint32_t allocate_block(ext2_fs_t* fs) {
    ...
    for (uint32_t group = 0; group < fs->groups_count; group++) {
        ...
        uint32_t blocks_in_group = (group == fs->groups_count - 1) ? 
            (fs->total_blocks - group * fs->blocks_per_group) : fs->blocks_per_group;
        ...
        for (uint32_t i = 0; i < blocks_in_group; i++) {
            if ((bitmap[i / 8] & (1 << (i % 8))) == 0) {
                ...
                kmemcpy((uint8_t*)bgdt_block + (group % (fs->block_size / sizeof(struct ext2_bg_desc))) * sizeof(struct ext2_bg_desc), 
...
static uint32_t allocate_inode(ext2_fs_t* fs, int is_directory) {
    ...
    for (uint32_t group = 0; group < fs->groups_count; group++) {
        ...
        for (uint32_t i = 0; i < fs->inodes_per_group; i++) {
            if (i == 0) continue; // Inode 0 is reserved
