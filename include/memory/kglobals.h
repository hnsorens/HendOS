 ...
 * @param last_global Previous global variable in the allocation chain
 */
#define __global_array_1___(type, count, last_global)                                              \
    ((type*)(OFFSET(last_global) - sizeof(type) * count))
...
#define NUM_2MB_PAGES                                                                              \
    createGlobal(uint64_t, HEAP_DATA) ///< Total number of 2 Mb pages in system memory
#define NUM_4KB_PAGES                                                                              \
    createGlobal(uint64_t, NUM_2MB_PAGES) ///< Total number of 4 Kb pages in system memory
...
#define FREE_STACK_2MB_TOP                                                                         \
    createGlobal(uint32_t, FREE_STACK_4KB) ///< 2 Mb page allocation free stack top
#define FREE_STACK_4KB_TOP                                                                         \
    createGlobal(uint32_t, FREE_STACK_2MB_TOP) ///< 4 Kb page allocation free stack top
#define KERNEL_PAGE_TABLE createGlobal(page_table_t, FREE_STACK_4KB_TOP) ///< Kernels paging table
#define PROCESS_MEM_FREE_STACK                                                                     \
    createGlobalArray(uint16_t, 2056,                                                              \
                      KERNEL_PAGE_TABLE) ///< Free stack for kernel page table process entries
#define MEMORY_REGIONS                                                                             \
    createGlobalArray(MemoryRegion, 10,                                                            \
                      PROCESS_MEM_FREE_STACK) ///< Preboot allocated memory regions for kernel
#define PREBOOT_INFO                                                                               \
    createGlobal(preboot_info_t,                                                                   \
                 MEMORY_REGIONS) ///< Information gathered before exiting boot services
...
#define TSS                                                                                        \
    createGlobal(TSS64, PROCESS_COUNT) ///< TSS which contains the stack that is switched to when
                                       ///< interrupts happen
...
#define GRAPHICS_LAYERS                                                                            \
    createGlobalArray(Layer*, 128, INTEGRATED_FONT) ///< Graphics layers for rendering
#define GRAPHICS_CONTEXT                                                                           \
    createGlobal(GraphicsContext, GRAPHICS_LAYERS) ///< Contains all graphics information
#define GRAPHICS_LAYER_COUNT                                                                       \
    createGlobal(uint32_t, GRAPHICS_CONTEXT) ///< Number of active graphics layers
...
#define KEYBOARD_STATE                                                                             \
    createGlobal(keyboard_state_t, DEV) ///< Current keyboard state from interrupts
...
#define VCONS                                                                                      \
    createGlobalArray(vcon_t, VCON_COUNT,                                                          \
                      DEVICE_MANAGER) ///< Global Array of All Virtual Console Drivers

/* Syscalls */
typedef void (*syscall_fn)();
#define SYSCALLS                                                                                   \
    createGlobalArray(syscall_fn, 512, VCONS) ///< List of all syscall function pointers
